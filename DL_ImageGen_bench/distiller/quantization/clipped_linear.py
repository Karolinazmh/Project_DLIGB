...
class LearnedClippedLinearQuantizeSTE(torch.autograd.Function):
    ...
    def forward(ctx, input, clip_val, num_bits, dequantize, inplace):
        ...
        # scale_factor = asymmetric_linear_quantization_scale_factor(num_bits, 0, clip_val.data[0].numpy().tolist())
...
class PReLULearnedClippedLinearQuantizeSTE(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, clip_val, num_bits, dequantize, inplace):
        ctx.save_for_backward(input, clip_val)
        if inplace:
            ctx.mark_dirty(input)
        scale_factor = asymmetric_linear_quantization_scale_factor(num_bits, -1, clip_val.data[0])
        # scale_factor = asymmetric_linear_quantization_scale_factor(num_bits, 0, clip_val.data[0].numpy().tolist())
        output = clamp(input, -1, clip_val.data[0], inplace)
        output = linear_quantize(output, scale_factor, inplace)
        if dequantize:
            output = linear_dequantize(output, scale_factor, inplace)
        return output
...
class PReLUQuantization(nn.Module):
    def __init__(self, num_bits, clip_val, num_parameters=1, init=0.25, dequantize=True, inplace=False):
        super(PReLUQuantization, self).__init__()
    ...
    def __repr__(self):
        ...
        return '{0}(num_bits={1}, num_parameters={2}, clip_val={3}{4})'.format(self.__class__.__name__,
                                                                               self.num_parameters, self.num_bits,
                                                                               self.clip_val,
                                                                               inplace_str)


class PReLULearnedClippedLinearQuantization(nn.Module):
    def __init__(self, pr_weight, num_bits, init_act_clip_val, dequantize=True, inplace=False):
        super(PReLULearnedClippedLinearQuantization, self).__init__()
        self.num_bits = num_bits
        # self.clip_val = nn.Parameter(torch.Tensor([init_act_clip_val]))
        self.weight = nn.Parameter(torch.Tensor([init_act_clip_val]))
        self.dequantize = dequantize
        self.inplace = inplace
        scale_factor_pr = asymmetric_linear_quantization_scale_factor(num_bits, 0, 1)
        self.pr_weight = LinearQuantizeSTE.apply(pr_weight, scale_factor_pr, True, False)

    def forward(self, input):
        input = F.prelu(input, self.pr_weight)
        input = PReLULearnedClippedLinearQuantizeSTE.apply(input, self.weight, self.num_bits, self.dequantize,
                                                           self.inplace)
        return input

    def __repr__(self):
        inplace_str = ', inplace' if self.inplace else ''
        return '{0}(num_bits={1}, clip_val={2}{3})'.format(self.__class__.__name__, self.num_bits, self.weight,
                                                           inplace_str)
...
class LearnedClippedLinearQuantization(nn.Module):
    def __init__(self, num_bits, init_act_clip_val, dequantize=True, inplace=False):
        ...
        self.num_bits = num_bits
        # self.clip_val = nn.Parameter(torch.Tensor([init_act_clip_val]))
        self.clip_val = nn.Parameter(torch.Tensor([init_act_clip_val])).cuda()
        self.dequantize = dequantize
        self.inplace = inplace
...
def dorefa_quantize_param(param_fp, num_bits):
    ...
    " if only one weight(PReLU), only quantize it 2018-09-30 13:33:57 CKH "
    if param_fp.shape == torch.Size([1]):
        out = LinearQuantizeSTE.apply(param_fp, scale_factor, True, False)
    else:
        out = param_fp.tanh()
...
class DorefaQuantizer(Quantizer):
    ...
    def __init__(self, model, optimizer, bits_activations=32, bits_weights=32, bits_overrides=OrderedDict(),
        ...
        def relu_replace_fn(module, name, qbits_map):
            ...
            if bits_acts is None:
                return module
            " PReLU not have inplace argument 2018-09-26 14:16:15 CKH "
            return ClippedLinearQuantization(bits_acts, 1, dequantize=True, inplace=module.inplace)

        def prelu_replace_fn(module, name, qbits_map):
            bits_acts = qbits_map[name].acts
            if bits_acts is None:
                return module
            " clip range [-1,1] to match PReLU 2018-09-30 10:41:06 CKH "
            return PReLUQuantization(bits_acts, 1, num_parameters=1, init=module.weight.data.tolist()[0],
                                     dequantize=True, inplace=False).cuda()

        self.param_quantization_fn = dorefa_quantize_param

        self.replacement_factory[nn.ReLU] = relu_replace_fn

        self.replacement_factory[nn.PReLU] = prelu_replace_fn
...
class PACTQuantizer(Quantizer):
    ...
    def __init__(self, model, optimizer, bits_activations=32, bits_weights=32, bits_overrides=OrderedDict(),
        ...
        def prelu_replace_fn(module, name, qbits_map):
            ...
            if bits_acts is None:
                return module
            " clip range [-1,clip_val] to match PReLU 2018-09-30 15:21:15 CKH "
            return PReLULearnedClippedLinearQuantization(pr_weight=module.weight, num_bits=bits_acts,
                                                         init_act_clip_val=act_clip_init_val,
                                                         dequantize=True, inplace=False).cuda()
