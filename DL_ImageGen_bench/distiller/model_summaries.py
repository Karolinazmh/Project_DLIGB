...
def conv_visitor(self, input, output, df, model, memo):
    ...
    # add group to cal weight vol, 2018-12-04 10:00:33 by CKH
    weights_vol = self.out_channels * self.in_channels / self.groups * self.kernel_size[0] * self.kernel_size[1]
...
# add transposeConv Macs calculate, copy from conv_visitor(), 2018-10-18 17:36:59 by CKH
def deconv_visitor(self, input, output, df, model, memo):
    assert isinstance(self, torch.nn.ConvTranspose2d)
    if self in memo:
        return
...
def model_performance_summary(model, dummy_input, batch_size=1):
    ...
    def install_perf_collector(m):
        if isinstance(m, torch.nn.Conv2d):
            ...
            hook_handles.append(m.register_forward_hook(
                                    partial(deconv_visitor, df=df, model=model, memo=memo)))
