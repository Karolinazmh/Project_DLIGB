...
from apputils.Deblur_apputils.image_pool import ImagePool
...
class ConditionalGAN(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf,
                                      opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids, use_parallel, opt.learn_residual, opt.rnn, opt.expend_ends_chnl, opt.depthwise, opt.shufflenet, opt.depthwisedeconv, opt.deconv2upsample)
        if self.isTrain and opt.which_model_netD != 'NoD':  # modified by KaiKang, add option "NoD"
        ...
        # if self.isTrain and opt.which_model_netD != 'NoD':
    ...
    def backward_G(self, opt):
        ...
        self.featureMap = self.contentLoss.get_feature(self.real_B)

    # split backward_G() to get_G_loss_func() & backward_G_func() 2018-10-16 17:36:10 CKH
    def get_G_loss_func(self, opt):
        if opt.which_model_netD != 'NoD':  # modified by KaiKang, add option "NoD"
            self.loss_G_GAN = self.discLoss.get_g_loss(self.netD, self.real_A, self.fake_B)
        else:
            self.loss_G_GAN = 0
    ...
    def get_current_errors(self, opt):
        # fix the warning invalida index of a 0-dim tensor 2018-10-16 17:36:10 CKH
        if opt.which_model_netD != "NoD":  # modified by KaiKang, add option "NoD"
    ...
    def save(self, label, opt):
        self.save_network(self.netG, 'G', label, self.gpu_ids)
        if opt.which_model_netD != 'NoD':  # modified by Kai Kang, add option "NoD"
